...
// Agregar una reseña
exports.addReseña = (req, res) => {
  const { idProductos, comentario, puntuacion } = req.body;
  console.log(idProductos,comentario, puntuacion);

  db.query('INSERT INTO ReseñaProducto (comentario,puntuacion,idProductos) VALLUES (?,?,?)',[comentario,puntuacion,idProductos],(err,result)=>{
    if(err){
      return res.json({error: "Error al insertar la reseña de dicho producto"});
    }
    
    res.json({ message: "Reseña agregada exitosamente", idReseñaProducto: result.insertId });
